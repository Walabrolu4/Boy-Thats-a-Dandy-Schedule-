...
**Past-week navigation bound to real history:** previously the back arrow was unbounded - you could navigate arbitrarily far into the past even for weeks with no saved data, which would just show an empty/default schedule. Added `hasWeekData(offset)` in `storage.js` (checks whether `ls-week-*` exists for that offset; offset 0 always counts as existing). `setWeekOffset(delta)` in `store.svelte.js` now refuses to go further back than the last week with saved data, and `WeekGrid.svelte`'s "◀" button is disabled once `hasWeekData(weekOffset - 1)` is false. Added `hasWeekData: () => true` to the `storage.js` mock in `tests/touch.test.js`. 12/12 tests pass.

**Micro session styling:** micro sessions (e.g. `drawmicro`) previously rendered at reduced opacity. Now they render identically to regular sessions but with a dashed border instead of solid - changed in the session `<div>`'s inline `style` in `WeekGrid.svelte` (border style is `dashed` when `session.micro`, `solid` otherwise; inline style was overriding any CSS-class-based border, so the fix had to be inline).

### Sprint 13.8 — Habit Onboarding Wizard
Implemented all task groups:
- New `src/lib/onboarding.js`: `buildTagsFromHabits(habits)` and `buildScheduleFromHabits(habits)` turn a `{ name, frequency (1-7), preferredTime }` list into `DEFAULT_TAGS`/`DEFAULT_DAYS`-shaped data. Frequency 7 = "Every day" (`mvwTarget:7, mvwOutOf:7`), `N>1` → `mvwTarget:N, mvwOutOf:7`, `N===1` → `mvwTarget:1` only. Occurrences spread evenly across the 7 days (`step = 7/frequency`), then each day's sessions are sorted Morning→Afternoon→Evening→Night.
- New `src/components/OnboardingWizard.svelte`: 2-step modal (habit list builder → per-day preview), styled like `SeasonalWizard`. Has a `dismissable` prop - `false` for the mandatory first-run wizard (no × button, click-outside is a no-op), `true` when reopened from Settings.
- `storage.js`: added `isOnboarded()`/`markOnboarded()` (the `ls-onboarded` flag) and `shouldShowOnboarding()` - only true when the flag is unset **and** no `ls-schedule`/`ls-tags`/`ls-week-*` keys exist yet. This is deliberately stricter than "just check the flag" so existing users (who have week history but never had this flag) and second devices hydrating an existing cloud account (whose `importData()` writes those keys before this check runs) never get unexpectedly prompted.
- `App.svelte`: after `syncEngine.hydrate()` in `onMount`, shows `OnboardingWizard` if `shouldShowOnboarding()`.
- `SettingsModal.svelte`: new "✨ Re-run Habit Setup" button (Schedule Advanced section) with a `confirm()` guard, opens the wizard with `dismissable={true}` and reloads on completion.
- `buildTasksFromHabits` from the original plan was dropped - onboarding never calls `saveTasks`, so existing/default tasks are left untouched (per the sprint's own fallback note).
- New `tests/onboarding.test.js` (8 tests): tag generation per frequency tier, schedule shape/day-distribution/time-of-day ordering/session-id numbering, and the onboarding-flag/`shouldShowOnboarding` logic. All 22 Vitest tests pass.

**Not yet verified in-browser** - dev server was started and stopped without a manual click-through (no browser automation available in this session). Worth manually testing: clear `localStorage` (or use a private window) and reload to confirm the first-run wizard appears, plus the "Re-run Habit Setup" flow from Settings.

**Follow-up: first run is now mandatory.** "Skip setup" only renders when `dismissable={true}` (i.e. reopened from Settings) - on first run (`dismissable={false}`, the default) there's no skip path and no × button, so a fresh install with no saved data must add at least one habit and click "Create my week" to proceed. The empty-habits validation message is also worded differently depending on `dismissable`.

**Follow-up: sync conflict prompt + sign-in during onboarding.**
- `SyncEngine.hydrate({ checkConflict })`: when `refreshAuthSubscription` detects a fresh sign-in, it now calls `hydrate({ checkConflict: true })`. If the cloud copy's `tags`/`schedule`/`tasks`/`theme` differ from this device's (JSON-string compare), and `shouldShowOnboarding()` is false, it awaits a registered `onConflict()` callback (resolves `'local'`/`'cloud'`) before importing - choosing `'local'` keeps this device's blobs and pushes the merged result back to the cloud. Per-week (`ls-week-*`) data is unaffected - it always goes through the existing CRDT `mergeState`.
- New `src/components/SyncConflictModal.svelte`, registered via `syncEngine.onConflict(...)` in `App.svelte`'s `onMount` - shows "Use synced data" vs "Keep this device's data" and resolves the pending promise.
- If `shouldShowOnboarding()` is still true at sign-in time (fresh/onboarding device with no real local data), the conflict prompt is skipped entirely and the cloud copy wins automatically - "favor cloud" during onboarding.
- `OnboardingWizard.svelte` (first-run only, `!dismissable`): added a collapsible "Already using Dandy Sync on another device? Sign in instead" section with an email + magic-link button (forces `syncConfig.provider = 'supabase'` if not already set). On successful sign-in, the resulting hydrate either pulls the existing cloud schedule (skipping onboarding) or, if the cloud account has no data yet, leaves onboarding to proceed normally.
- All 22 Vitest tests still pass; not yet verified in-browser (no real Supabase session available in this session).

**Sync status dot now reflects "sync off".** `SyncEngine.emitStatus()` previously always reported `'synced'` (🟢) once `pending`/`syncing`/`offline` were all false, even when `syncConfig.provider === 'none'` or the user wasn't signed in/configured - i.e. the dot was green even with cloud sync never set up. Added a new `'disabled'` status (rendered as ⚪, tooltip "Cloud sync is off") returned whenever there's no active adapter: `provider === 'github'` with no token, or `provider === 'supabase'` with no signed-in `supabaseUser`. 🟢 now only appears once a sync provider is actually configured and caught up.

**Fixed: editing tags after onboarding reverted to old default tags and orphaned scheduled sessions' `tagId`s.** `MvwChips.svelte` read `getTagsSync()` into `tags` `$state` once at mount - since `MvwChips` is mounted before the mandatory first-run onboarding wizard runs, its `tags` stayed on the pre-onboarding defaults. Any subsequent edit called `saveConfig()` → `saveTags(tags)`, overwriting the onboarding-generated `ls-tags` with that stale default array, leaving schedule sessions' `tagId`s pointing at tags that no longer existed. Added an `$effect` that resyncs `tags = getTagsSync()` whenever `globalStore.scheduleVersion` changes (bumped by `incrementScheduleVersion()` after onboarding/hydrate/etc.). Only prevents future occurrences - existing installs already in the broken state need to re-run "✨ Re-run Habit Setup" or manually re-tag sessions. 22/22 Vitest tests pass.

### Sprint 13.5 (remainder) — Profile Schema & Account UI
- New `supabase/profiles_schema.sql`: `public.profiles` table (`user_id` PK → `auth.users`, `display_name`, `avatar_url`, `updated_at`) with RLS scoped to `auth.uid() = user_id`, plus a public `avatars` storage bucket and `storage.objects` RLS policies (public read; insert/update/delete restricted to each user's own `<user_id>/...` path prefix via `storage.foldername(name)`). **Manual step required**: run this file once in the Supabase SQL editor (alongside the existing `schema.sql`/`lock_dandy_sync_email.sql`) before the Account UI will work.
- `SupabaseAdapter.js`: added `getProfile()` (returns `{ display_name, avatar_url }`, defaulting to nulls if no row yet), `updateProfile({ display_name })` (upsert), `uploadAvatar(file)` (uploads to `avatars/<uid>/avatar.<ext>` with `upsert: true`, updates `profiles.avatar_url` with the public URL, returns it), and `removeAvatar()` (removes common-extension avatar objects from storage and nulls `avatar_url`).
- `SettingsModal.svelte`: new "Account" section (only shown when `syncConfig.provider === 'supabase'`), placed right after the Sync section. Signed-out: hint pointing back to the Sync section's magic-link form (deliberately not duplicated here). Signed-in: avatar preview (circular image or an initial-letter placeholder), "Change Avatar"/"Remove" buttons (hidden file input, mirroring the existing data-import pattern), and a display-name text input that autosaves via `updateProfile` on blur, with a status line for save/upload errors. Profile is loaded once via an `$effect` keyed on `supabaseUser`/`syncConfig.provider`. New `.profile-row`/`.avatar-preview`/`.avatar-placeholder`/`.avatar-actions` styles added.
- Not yet verified in-browser (requires running `profiles_schema.sql` against the live Supabase project first).
